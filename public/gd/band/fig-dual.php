<?php
	...
	function generate_fig_dual($colors)
	{
		...
		if (!file_exists("img/custom/figured/dual/" . $name . ".png")) {
			...
			if (!file_exists('img/custom/figured/dual')) {
				mkdir('img/custom/figured/dual', 0777, true); // If not, then create directory path
			}
			// Make image
            imagepng( $img_0_dst, "img/custom/figured/dual/" . $name . ".png" );

			imagedestroy( $img_0_dst );

		}

		echo( "/gd/img/custom/figured/dual/" . $name . ".png" );
